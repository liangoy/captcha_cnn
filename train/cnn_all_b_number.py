...
x_mean=tf.reshape(tf.tile(tf.reduce_mean(x,axis=[1,2]),[30*80]),shape=[batch_size,30,80])

X =tf.nn.relu( tf.abs(x- x_mean)-30)
XX=X/(X+0.0001)

c1 = ml.conv2d(tf.expand_dims(XX, axis=-1), conv_filter=[5, 5, 1, 16], ksize=[1, 3, 4, 1], pool_stride=[1, 3, 4, 1],
               nn=tf.nn.relu)  # [10,20 ]
c2 = ml.conv2d(c1, conv_filter=[3, 4, 16, 32], ksize=[1, 2, 4, 1], pool_stride=[1, 2, 4, 1], nn=tf.nn.relu)  # [5,5]
...
out = tf.nn.relu(tf.layers.batch_normalization(tf.reshape(c3, shape=[batch_size, 128]), training=training))
y = tf.nn.softmax(ml.layer_basic(out, 10 * 4))
...
for i in range(10 ** 10):
    ...
    if i % 10 == 0:
        test_y, test_x = generate_number_image(batch_size)
